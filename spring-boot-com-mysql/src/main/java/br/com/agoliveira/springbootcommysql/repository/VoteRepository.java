...
import br.com.agoliveira.springbootcommysql.model.ChoiceVoteCount;
import br.com.agoliveira.springbootcommysql.model.Vote;
...
public interface VoteRepository extends JpaRepository<Vote, Long> {
    @Query("SELECT NEW br.com.agoliveira.springbootcommysql.model.ChoiceVoteCount(v.choice.id, count(v.id)) FROM Vote v WHERE v.post.id in :postIds GROUP BY v.choice.id")
    List<ChoiceVoteCount> countByPostIdInGroupByChoiceId(@Param("postIds") final List<Long> postIds);

    @Query("SELECT NEW br.com.agoliveira.springbootcommysql.model.ChoiceVoteCount(v.choice.id, count(v.id)) FROM Vote v WHERE v.post.id = :postId GROUP BY v.choice.id")
